...
class LeakyRelu:
    ...
    def forward(self, x):
        ...
        output_bigger_than_zero = np.maximum( x, 0)
        outputs = (output_bigger_than_zero == 0)*self.slope*x + output_bigger_than_zero
    ...
    def backward(self, dout, cache):
        ...
        dx = dout * ((cache > 0)+(cache < 0)*self.slope)
...
class Tanh:
    ...
    def forward(self, x):
        ...
        sigmoid_2x = outputs = 1 / (1 + np.exp(-2*x))
